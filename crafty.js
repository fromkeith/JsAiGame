...
Crafty.extend({
	...
	*/
	sprite: function (tile, tileh, url, map, paddingX, paddingY) {
		var spriteName, temp, x, y, w, h, img, options;
		...
		for (spriteName in map) {
			...
			Crafty.c(spriteName, {
				...
				init: function () {
					...
					this.__repeat = this.__options["repeat"];
					this.img = img;
			...
			if (e.type === "canvas") {
				...
				if(!this.__repeat) {
					context.drawImage(this.img, //image element
			 		...
			 		for (var x = 0; x < pos._w; x += co.w){
			 			for(var y=0; y < pos._h; y += co.h) {
			 				var tileHeight = co.h;
			 				if (y + co.h > pos._h) {
			 					tileHeight = pos._h - y; 
			 				}
				 			...
				 			context.drawImage(this.img, //image element
										 ...
										 co.w, //width on sprite
										 co.h, //height on sprite
										 pos._x + x, //x position on canvas
										 pos._y + y, //y position on canvas
										 tileWidth, //width on canvas
										 tileHeight //height on canvas
										 );
						}
			 		}
			 	}
